...
%%    shuwa_http_server:bind(<<"/swagger_map.json">>, ?MODULE, [], priv)
swagger_map() ->
    [
        shuwa_http_server:bind(<<"/swagger_map.json">>, ?MODULE, [], priv)
    ].
...
%% OperationId:map
%% 请求:GET map
do_request(get_map, Arg, Context, _Req) ->
    case dgiot_map:get_map(Arg, Context) of
        {ok, Success} ->
            {ok, Success};
        {error, Reason} ->
            {400, Reason}
...
%% map 概要: 发送组态数据
%% OperationId:map
%% 请求:post post_send_map
do_request(post_map, Arg, Context, _Req) ->
    case dgiot_map:put_map(Arg, Context) of
        {ok, Success} ->
            {ok, Success};
        {error, Reason} ->
            {400, Reason}
...
do_request(get_konva_thing, Arg, Context, _Req) ->
    case dgiot_map:get_konva_thing(Arg, Context) of
        {ok, Success} ->
            {ok, Success};
        {error, Reason} ->
            {400, Reason}
...
do_request(post_konva_thing, Arg, Context, _Req) ->
    case dgiot_map:edit_konva(Arg, Context) of
        {ok, Success} ->
            {ok, Success};
        {error, Reason} ->
            {400, Reason}
